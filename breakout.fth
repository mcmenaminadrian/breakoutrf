...
DECIMAL 10 CONSTANT MAXATTEMPTS
VARIABLE ATTEMPTS
VARIABLE CURRENTBALLDELAY
VARIABLE OLDBALLXY 1 CELLS ALLOT

\ random number generation stuff
DECIMAL 4 CONSTANT MODULUS
DECIMAL 3 CONSTANT MULTIPLIER
DECIMAL 2 CONSTANT INCREMENT
VARIABLE CURRENT_RANDOM

: GETRAND
  ( -- )
  CURRENT_RANDOM @ MULTIPLIER *
  INCREMENT +
  MODULUS MOD
  CURRENT_RANDOM !
;
...
: DRAWBOUNDARY    \ draw a boundary wall
 ...
 LOOP
 0 0 [ DECIMAL 32 ] LITERAL MVADDCH ERRORDETECT
 WIDTH 0 DO
...
: STARTGAME
 ...
 \ fixed for now
 CURRENT_RANDOM @ 1+ BALLXYV !
 GETRAND
 [ DECIMAL 20 ] CURRENT_RANDOM @ - LITERAL BALLXYV 1 CELLS + !
 GETRAND
 CURRENT_RANDOM @  BALLXYV 2 CELLS + !
 GETRAND
 0 CURRENTBALLDELAY !
 DRAWBALL
;
...
: UPDATEBOARD
  ( -- )
  0 COLOR_PAIR ATTRON
  0 WIDTH 5 + MOVESTD ERRORDETECT
  ATTEMPTS @ S\" ATTEMPT:  %d"
  PRINTW ERRORDETECT
  1 WIDTH 5 + MOVESTD ERRORDETECT
  SCORE @ S\" SCORE:  %d"
  PRINTW ERRORDETECT
  2 WIDTH 5 + MOVESTD ERRORDETECT
  BLOCKCOUNT @ S\" BLOCKS:  %d"
  PRINTW ERRORDETECT
;
...
: GAME
 ...
 BEGIN
   ...
   -1 = IF DROP ELSE 
     ...
     PROCESSKEYBAT
   THEN
   [ DECIMAL 20 ] LITERAL ATTEMPTS @ - MS
   PROCESSBALL
   CHECKOUTOFBOUNDS
   IF
     CLEANBALL
